...
import {
  ...
  Search, ChevronDown, Globe, Heart, Sparkles, MoreHorizontal,
} from "lucide-react";
...
// Primary nav — always shown inline when there's space
const PRIMARY_NAV = [
  { to: "/",      label: "Home" },
  { to: "/shop",  label: "Shop" },
...
// Secondary nav — shown in a "More" dropdown to avoid overlap
const SECONDARY_NAV = [
...
export function Header() {
  ...
  useEffect(() => {
    if (!regionOpen && !moreOpen) return;
    function onDown(e: MouseEvent) {
      if (regionOpen && regionRef.current && !regionRef.current.contains(e.target as Node)) {
        setRegionOpen(false);
      }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex justify-between items-center h-9 border-b border-border text-[11px] text-muted-foreground">
          <div className="flex items-center gap-2 min-w-0">
            <Sparkles className="size-3.5 text-hive shrink-0" />
            <span className="hidden sm:inline font-medium truncate">Free shipping over $50 · Member-only deals every week</span>
        ...
        {/* Main nav — uses flex with min-w-0 so items don't overflow */}
        <div className="flex justify-between items-center h-16 sm:h-[72px] gap-3">
          ...
          <div className="flex items-center gap-4 lg:gap-6 min-w-0 flex-shrink">
            ...
            <nav className="hidden lg:flex items-center gap-4 xl:gap-5 text-sm font-medium text-muted-foreground min-w-0">
              ...
              {/* More dropdown — contains secondary nav items */}
              <div className="relative shrink-0" ref={moreRef}>
                ...
                {moreOpen && (
                  <div className="absolute left-0 top-full mt-2 w-44 bg-card border border-border rounded-xl shadow-card overflow-hidden animate-scale-in z-50">
                    {SECONDARY_NAV.map((item) => (
                      <Link
                        key={item.to}
                        to={item.to}
                        onClick={() => setMoreOpen(false)}
                        className="block px-4 py-2.5 text-sm hover:bg-muted/60 transition-colors"
                        activeProps={{ className: "text-hive font-semibold bg-hive/5" }}
                        activeOptions={{ exact: item.to === "/" }}
                      >
                        {item.label}
                      </Link>
          ...
          {/* Right cluster — search + actions + cart */}
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            {/* Search — flex-1 with min-w-0 so it shrinks properly instead of overlapping */}
            <form onSubmit={onSearch} className="hidden md:flex items-center relative min-w-0">
              ...
              <input
                ...
                className="w-36 sm:w-44 lg:w-56 xl:w-64 pl-9 pr-3 py-2 rounded-full bg-secondary border border-border text-sm focus:outline-none focus:ring-2 focus:ring-hive/40 focus:border-hive/40 focus:w-64 xl:focus:w-72 transition-all"
              />
            </form>
            ...
            {user ? (
              <>
                {isAdmin && (
                  <Link to="/admin" className="hidden md:flex items-center gap-1.5 text-xs font-semibold px-3 py-2 border border-hive/40 text-hive rounded-full hover:bg-hive/10 transition-colors whitespace-nowrap">
                    <Shield className="size-3.5" />
                    <span className="hidden xl:inline">Admin</span>
                  </Link>
                )}
                <Link to="/dashboard" className="hidden md:flex items-center gap-1.5 text-xs font-semibold px-3 py-2 border border-border rounded-full hover:bg-secondary transition-colors whitespace-nowrap">
              ...
              <Link
                to="/login"
                className="flex items-center gap-1.5 text-xs font-semibold px-3 sm:px-4 py-2 border border-border rounded-full hover:bg-secondary transition-colors whitespace-nowrap"
              >
            ...
            <Link
              to="/checkout"
              className="relative bg-hive text-white px-3 sm:px-4 py-2 sm:py-2.5 rounded-full group cursor-pointer overflow-hidden hover-glow flex items-center gap-1.5 sm:gap-2 shrink-0"
            >
        ...
        <div className="hidden lg:flex items-center gap-1 h-11 border-t border-border overflow-x-auto scrollbar-hide">
          ...
          {QUICK_CATEGORIES.map((c) => (
            <Link
              ...
              className="px-3 py-1.5 rounded-full text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors whitespace-nowrap flex items-center gap-1.5 shrink-0"
            >
              <span>{c.icon}</span>
              <span>{c.label}</span>
            </Link>
